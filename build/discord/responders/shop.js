...
createResponder({
  ...
  async run(interaction, { itemId, userId }) {
    ...
    if (!item) {
      await interaction.reply({ content: "\u274C Este art\xEDculo ya no existe.", ephemeral: true });
      return;
    }
    ...
    await interaction.reply({
      embeds: [
        new ThemedEmbed(interaction).setTitle("\u2705 Compra Exitosa").setDescription(`Has comprado **${item.itemName}** por **$${itemPrice.toLocaleString()}**.`).setColor("#2ecc71")
      ],
      ephemeral: true
    });
  }
