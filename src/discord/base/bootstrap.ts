...
export async function loadModules(workdir: string, modules: string[] = [], nocache = false) {
    const app = Constatic.getInstance();
    ...
    for (const path of files) {
        ...
        // Detect category from path (e.g., discord/commands/ECONOMY/work.ts)
        ...
        await import(nocache ? `${url}?t=${Date.now()}` : url);
    }
    app.currentCategory = null;
}
